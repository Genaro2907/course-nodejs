...
export class UploadFileService {
    ...
    async upload(base64: string): Promise<string> {
        ...
        if(!fileType) {
            throw new ValidationError("A extensão do arquivo nao é válida!");
        }

        if(fileType.mime !== "image/jped" && fileType.mime !== "image/png") {
        ...
        const bucket = getStorage().bucket("e-commerce-3a1e7.firebasestorage.app");
